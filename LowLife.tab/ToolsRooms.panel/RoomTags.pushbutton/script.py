...
__doc__ = (
    ...
    u"загруженные марки категории «Марки помещений»). Битые марки "
    u"(показывают «?»: АР удалил/переделал помещение, марка осиротела "
    u"или помещение не окружено) пересоздаются на месте, а если "
    u"помещения под маркой уже нет — удаляются. На время вставки "
    u"временно снимается подложка вида — она мешает Revit ставить марки "
    u"помещений.\n\n"
