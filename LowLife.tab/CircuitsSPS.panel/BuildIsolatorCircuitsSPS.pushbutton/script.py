...
__doc__ = (
    u"Строит электрические цепи между изолятором и выбранными устройствами "
    u"пожарной сигнализации: выберите устройства и изолятор («Электрооборудование») "
    u"вместе одним выбором — среди выбранного должен быть ровно один изолятор. "
    u"После подтверждения (Enter/«Готово») цепи строятся сразу, и кнопка тут же "
    u"предлагает выбрать следующий набор — повторный запуск кнопки не нужен, "
    u"выбор заканчивается по Esc. "
    u"Все выбранные устройства ручного пуска (с «ручной» в имени типа — ИПР) "
    u"объединяются в одну общую цепь (в одной цепи может быть несколько ИПР), "
    u"остальные выбранные устройства — во вторую общую цепь. У изолятора "
    u"максимум 2 цепи — «ручная» и «автоматическая»."
)
__author__ = "Pipers"
