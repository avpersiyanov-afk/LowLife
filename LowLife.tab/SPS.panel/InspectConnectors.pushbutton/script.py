...
__doc__ = (
    u"Тестовая кнопка: выберите панель, затем несколько устройств шлейфа. "
    u"Показывает категорию и электрические коннекторы каждого элемента "
    u"(Domain, ConnectorType) — помогает понять, почему ElectricalSystem.Create "
    u"отказывается создавать цепь заданного типа: как правило, среди "
    u"переданных элементов должен быть хотя бы один категории "
    u"«Электрооборудование» (OST_ElectricalEquipment) — обычные устройства "
    u"(датчики и т.п.) сами по себе цепь создать не могут, только быть "
    u"нагрузкой в ней."
)
__author__ = "Pipers"
