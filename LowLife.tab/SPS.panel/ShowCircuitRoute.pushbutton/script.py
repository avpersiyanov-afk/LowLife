...
__title__ = u"Маршрут\nцепи"
__doc__ = (
    u"Показывает маршрут выбранного шлейфа СПС на модели — для визуальной "
    u"проверки топологии (в т.ч. ветвей от изоляторов). Выберите устройство, "
    u"входящее в интересующий шлейф (если оно состоит в нескольких цепях — "
    u"предложит выбрать нужную из списка). Строит временные линии (Detail "
    u"Line на активном виде) по рёбрам дерева шлейфа — панель -> первое "
    u"устройство и родитель -> устройство для каждого звена (не непрерывная "
    u"цепочка, как у СКС/СКУД: у СПС узлом шлейфа служит само устройство, а "
    u"ветви от изоляторов не возвращаются в магистраль) — и выделяет все эти "
    u"устройства в модели. Вид сам масштабируется, чтобы весь шлейф "
    u"поместился целиком. Линии временные: удаляются сами через несколько "
    u"секунд, а также при повторном запуске кнопки (в т.ч. кнопки СКС/СКУД "
    u"с той же функцией) — прошлые линии удаляются в любом случае."
)
__author__ = "Pipers"
