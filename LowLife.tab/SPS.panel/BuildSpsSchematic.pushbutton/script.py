...
__doc__ = (
    ...
    "если он заполнен (и, если задан «Фильтр «Имя лота»», содержит эту "
    "подстроку без учёта регистра — на случай если тот же параметр на "
    "помещениях используется не только под нужные на схеме лоты, но и "
    "под более широкие группировки), его значение принудительно "
    "записывается в параметр помещения устройства (и на реальный элемент "
    "в модели, и на схемный узел) вместо обычного имени/номера — так "
    "несколько физически разных помещений, отмеченных одним «лотом», на "
    "схеме читаются одной общей рамкой. У изолятора без такой цепи "
    "ответвления нет — его устройства остаются в общей раскладке по "
    "помещению как обычно.\n\n"
