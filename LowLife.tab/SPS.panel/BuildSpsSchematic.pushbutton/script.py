...
__doc__ = (
    u"Строит/обновляет структурную схему СПС (пожарная сигнализация и "
    u"электрооборудование — панели/изоляторы). Сама находит на модели все "
    u"устройства, тип которых сопоставлен категории в настройках СПС, "
    u"группирует их по этажу (подпись — «Этаж N (отметка)», порядок по "
    u"отметке: отрицательные — внизу схемы, глубже — ниже; положительные — "
    u"выше, чем больше отметка) и по помещению.\n\n"
    u"Повторный запуск не пересоздаёт схему с нуля: обновляется вид с именем "
    u"из настроек СПС (создаётся с этим именем, если его ещё нет), раскладка "
    ...
    u"Ответвления изоляторов (устройства, подключённые к изолятору — состав "
    u"берётся из фактической электрической цепи «изолятор -> устройства», "
    u"кнопка «Цепи изолятор-устройства СПС», а не из адреса или геометрии) "
    u"рисуются НЕ в своём обычном помещении, а отдельным рядом-веткой прямо "
    u"под изолятором на схеме (тот же шаг между узлами и та же рамка, что у "
    u"обычного помещения, зазор 5 мм), — итог похож на дерево, растущее из "
    u"кольца. Если ВСЕ устройства ветви — в том же помещении, что и сам "
    u"изолятор, отдельная рамка не рисуется — ветка встаёт прямо под "
    u"изолятором внутри рамки его же помещения («Смещение ветки в том же "
    u"помещении, мм» в настройках СПС); место под неё (и по высоте, и по "
    u"ширине) резервируется заранее, вместе с общей раскладкой этажа. "
    u"Устройства ветви часто физически разбросаны по нескольким разным "
    u"помещениям — если в настройках задан параметр «Имя лота на помещении "
    u"в связанной модели», для таких устройств сначала смотрится этот "
    u"параметр у помещения связи, в которое попадает точка устройства, и, "
    u"если он заполнен (и, если задан «Фильтр «Имя лота»», содержит эту "
    u"подстроку без учёта регистра — на случай если тот же параметр на "
    u"помещениях используется не только под нужные на схеме лоты, но и "
    u"под более широкие группировки), его значение принудительно "
    u"записывается в параметр помещения устройства (и на реальный элемент "
    u"в модели, и на схемный узел) вместо обычного имени/номера — так "
    u"несколько физически разных помещений, отмеченных одним «лотом», на "
    u"схеме читаются одной общей рамкой. У изолятора без такой цепи "
    u"ответвления нет — его устройства остаются в общей раскладке по "
    u"помещению как обычно.\n\n"
    u"Если в настройках задана категория «Шкаф»/«Панель» — рисуются линии до "
    u"него шинной топологией: на каждом этаже один общий горизонтальный "
    u"коллектор чуть ниже узлов, от каждого узла к нему короткий вертикальный "
    u"отвод, коллекторы всех этажей выходят на один общий вертикальный "
    u"стояк слева от рамок этажей.\n\n"
    u"Кроме этого, для каждого шлейфа (адрес устройства «панель.шлейф.номер») "
    u"рисуется кольцевой шлейф: внутри каждого помещения, где есть устройства "
    u"этого шлейфа (включая панель, если она там же), — один отрезок на "
    u"высоте самих узлов, проходящий через них (без отхода вниз). Переход в "
    u"ЛЮБОЕ другое помещение — хоть соседнее, хоть на другом этаже — только "
    u"через один общий стояк слева от рамок этажей, никогда напрямую между "
    u"помещениями (иначе линия резала бы чужие рамки, оказавшиеся между "
    u"ними): от края помещения к стояку идёт короткий спуск в свободный "
    u"зазор под рамкой, а не по прямой через чужие помещения. Кольцевой "
    u"интерфейс идёт «туда» и «обратно» по одному и тому же маршруту, "
    u"поэтому вся структура (отрезки внутри помещений + спуски + стояк) "
    u"рисуется в два прохода со сдвигом 3 мм между ними. Шлейф без панели "
    u"среди узлов схемы (панель не сопоставлена ни одной категории в "
    u"настройках) рисуется без замыкания на неё.\n\n"
    u"Оба вида линий (к шкафу и кольца/ответвления) не редактируются "
    u"вручную — на каждом запуске перерисовываются заново по актуальным "
    u"позициям, и оба выключаются ОДНИМ полем настроек. На больших моделях "
    u"их построение заметно дольше, чем сама раскладка узлов по этажам/"
    u"помещениям (включая ряды-ветки ответвлений — те строятся всегда, "
    u"линий не касается). Чтобы сначала проверить/поправить раскладку без "
    u"ожидания линий, впишите «нет» в настройках СПС в поле «Рисовать "
    u"кольцевые шлейфы и ответвления» — раскладка построится как обычно, "
    u"линии просто не рисуются (старые, если были, удаляются). Верните «да» "
    u"(или очистите поле), когда раскладка устроит, и запустите ещё раз — "
    u"линии достроятся.\n\n"
    u"Марки узлов (IndependentTag) — тоже отдельный переключатель, «Ставить "
    u"марки узлов»: сама вставка марки — штатный вызов Revit API, который на "
    u"некоторых моделях занимает секунду и больше НА КАЖДУЮ марку, и легко "
    u"становится основным временем построения схемы при сотнях устройств "
    u"(на практике наблюдалось на порядок дольше, чем всё остальное вместе "
    u"взятое). Впишите «нет», чтобы сначала построить/поправить раскладку "
    u"без марок, добавить их отдельным запуском, когда раскладка устроит."
)
__author__ = "Pipers"
...
from lowlife.room_info import (
    get_point as get_room_point, find_room_value, find_room_param_value
)
...
BRANCH_LOT_FILTER_WORD = settings.get("branch_lot_filter_word") or u""
...
def resolve_room_value(doc, el, counters, is_branch=False):
    ...
    заполнен (и, если задан BRANCH_LOT_FILTER_WORD, содержит эту
    подстроку без учёта регистра — см. её докстринг в настройках: тот же
    параметр на помещениях может использоваться не только под лоты,
    которые нужны на схеме как есть, но и под более широкие группировки,
    которые там не нужны), ПРИНУДИТЕЛЬНО (даже если ROOM_PARAM_NAME на
    устройстве уже чем-то заполнен) записываем его значение в
    ROOM_PARAM_NAME и возвращаем, минуя обычный поиск имени/номера
    помещения. Устройства ветви физически часто разбросаны по нескольким
    помещениям — обычный геометрический поиск по каждой точке отдельно
    совершенно корректно находит им разные помещения, а "Имя лота" —
    способ вручную указать в связанной модели, что несколько таких
    помещений на самом деле одна логическая зона ветки. Если параметр не
    задан в настройках, у найденного Room пуст, или не проходит фильтр —
    просто продолжаем обычной логикой ниже (эта ветка не отличается от
    ...
    if is_branch and BRANCH_LOT_ROOM_PARAM_NAME:
        ...
        lot_matches = lot_value and (
            not BRANCH_LOT_FILTER_WORD or BRANCH_LOT_FILTER_WORD.lower() in lot_value.lower()
        )
        if lot_matches:
