...
__doc__ = (
    u"Строит/обновляет структурную схему СКС. Сама находит устройства через "
    u"уже настроенные цепи и панели (те же критерии, что и «Расчёт длины "
    u"цепи»), группирует их по этажу (подпись — «Этаж N (отметка)», порядок "
    u"по отметке: отрицательные — внизу схемы, глубже — ниже; положительные "
    u"— выше, чем больше отметка) и по помещению — рамки и раскладка "
    u"устроены так же, как у структурной схемы СОТ.\n\n"
    u"У каждой панели (шкафа/патч-панели) — своя независимая цветная шина: "
    u"свой горизонтальный коллектор на каждом этаже, где у неё есть "
    u"устройство (только её собственные отростки), свой вертикальный "
    u"стояк слева от рамок через все её этажи. Коллекторы и стояки разных "
    u"панелей на одном этаже разнесены — по высоте (коллектор) и по X "
    u"(стояк), не накладываются друг на друга. Отростки, коллектор и "
    u"стояк одной панели окрашены в один и тот же цвет (Line Style, "
    u"создаётся автоматически по имени панели) — видно, какое устройство "
    u"к какому шкафу идёт. Показаны только розетка и панель, без "
    u"промежуточных узлов трассы (без стояков/узлов маршрута из «Адреса "
    u"узлов»).\n\n"
    u"Магистральные связи шкаф-шкаф (цепь, у которой вместо устройства "
    u"оказалась другая целевая панель — например оптическая линия между "
    u"двумя шкафами) отходят от собственного узла каждого шкафа (не от "
    u"его шины устройств — на видимом расстоянии от неё) до своей "
    u"отдельной вертикальной дорожки слева от стояков панелей, ярким "
    u"жирным цветом; несколько связей через общий шкаф (цепочка A-B-C) "
    u"используют одну общую дорожку на всех.\n\n"
