...
__title__ = u"Маршрут\nцепи"
__doc__ = (
    u"Показывает маршрут выбранной цепи СКУД на модели — для визуальной "
    u"проверки, что маршрут выбран правильно. Выберите устройство, входящее "
    u"в интересующую цепь (если оно состоит в нескольких цепях — предложит "
    u"выбрать нужную из списка). Строит временную линию (Detail Line на "
    u"активном виде) через устройство, узлы/стояки маршрута (по данным "
    u"параметра «Маршрут цепи», записанного кнопкой «Длины и маркировка») и "
    u"контроллер, и выделяет все эти элементы в модели. Для устройств рядом "
    u"с контроллером (когда цепь посчитана по катетам, без узлов маршрута) "
    u"строит прямую линию устройство -> контроллер. Вид сам масштабируется, "
    u"чтобы весь маршрут поместился целиком. Линия временная: "
    u"удаляется сама через несколько секунд, а также при повторном запуске "
    u"кнопки (в т.ч. кнопки СКС/СПС с той же функцией) — прошлая линия "
    u"удаляется в любом случае."
)
__author__ = "Pipers"
