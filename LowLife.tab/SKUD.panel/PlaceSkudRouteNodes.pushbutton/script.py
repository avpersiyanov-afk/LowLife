# -*- coding: utf-8 -*-
__title__ = u"Узлы трассы\nСКУД"
__doc__ = (
    u"Расставляет панели/узлы маршрута/стояки СКУД в точках трассы кабеля. "
    u"Узлы СКУД — свои, отдельные от СКС: узлы СКС ведут до шкафа СКС, узлы "
    u"СКУД — от устройств до контроллера. Панели/стояки распознаются только "
    u"среди элементов рабочего набора СКУД."
)
__author__ = "Pipers"
...
def _open_settings():
    edited = get_settings_interactive(doc, keys=[
        ...
        "workset_filter_key", "workset_param_name", "cable_param_name",
        "route_param_name", "offset_param_names", "panel_keywords",
