...
__doc__ = (
    u"Строит и обновляет структурную схему СКУД на отдельном чертёжном виде "
    u"(имя — из настроек). Схемное семейство контроллера ставится один раз "
    u"на контроллер; на каждую точку прохода подбирается группа деталей, "
    u"чей состав устройств совпадает с составом точки прохода. Повторный "
    u"запуск — инкрементальная синхронизация: неизменные точки прохода не "
    u"трогаются (ручная раскладка сохраняется), изменившиеся перерисовываются, "
    u"пропавшие удаляются. Состояние — JSON в служебном параметре вида."
)
__author__ = "Pipers"
