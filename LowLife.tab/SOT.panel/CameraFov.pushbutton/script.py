...
__doc__ = (
    ...
    u"пропускает только «Охранную сигнализацию». Горизонтальный и "
    u"вертикальный угол обзора — ВСЕГДА расчётные, из фокусного "
    u"расстояния объектива и формата матрицы (не вписываются вручную); "
    u"дальность, высота установки и наклон берутся из параметров камеры "
    u"(имена — в настройках). Повторный запуск перерисовывает зоны тех же "
    u"камер, не плодя дубли.\n\n"
...
_STATUS_RU = {
    ...
    u"no_optics": u"нельзя посчитать угол обзора (фокусное/матрица)",
...
sot_fov.require(settings, ["distance_param_name", "focal_length_param_name", "zone_tag"])

_has_sensor = bool((settings.get("sensor_format") or u"").strip()
                   or (settings.get("sensor_format_param_name") or u"").strip())
if not _has_sensor:
    forms.alert(
        u"В настройках не задан формат матрицы — он нужен вместе с "
        u"фокусным расстоянием, чтобы посчитать угол обзора: заполните "
        u"«Формат матрицы» (общий на все камеры) или «Параметр формата "
        u"матрицы» (на самой камере).\n\n"
