...
__doc__ = (
    ...
    u"взгляда камеры. Если фокусное расстояние — параметр ЭКЗЕМПЛЯРА "
    u"камеры (вариофокальный объектив), заодно подбирает и его — так, "
    u"чтобы в кадр на дальней точке помещалась вся ширина помещения там "
    u"же. Сама зону не строит — после запуска перестройте её кнопкой "
    u"«Зоны обзора».\n\n"
    ...
    u"типа). Если получившиеся наклон или ближняя мёртвая зона выходят за "
    u"настроенный допустимый порог — отчёт предупредит, что камеру стоит "
    u"установить выше.\n\n"
    u"Shift+клик — настройки (те же, что у «Зон обзора», плюс значения "
    u"по умолчанию и диапазон/пороги автоподбора)."
)
__author__ = "Pipers"
...
_STATUS_RU = {
    ...
    u"ok_warn": u"наклон записан (с предупреждением)",
...
def _report(results):
    ...
    for cam, status, detail in results:
        is_ok = status in (u"ok", u"ok_warn")
