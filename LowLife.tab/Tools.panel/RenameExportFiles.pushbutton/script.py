...
__doc__ = (
    ...
    u"После команды ModPlus «Экспорт» тот же диалог появляется сам — "
    u"это делает хук hooks/command-after-exec.py.\n\n"
    u"Shift+клик — настройки: автозапуск после экспорта, подстрока-триггер "
    u"команды, что на что менять, расширения, подпапки."
)
__author__ = "Pipers"
