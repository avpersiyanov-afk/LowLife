...
оказались бы на схеме дважды) И приводит помещение ВСЕХ устройств ветки
к помещению изолятора (перезаписывает параметр помещения и на схемных
узлах, и на самих реальных устройствах в модели, поверх того, что
индивидуально нашёл по точке поиск помещения в связи для каждого из
них) — так что подпись рамки-спутника (см. ниже, sync_isolator_satellites)
почти всегда получает одно общее имя, а не список разных.
...
def sync_isolator_satellites(doc, view, old_satellite_ids, isolator_branches, new_state,
    ...
    for level_key, items in by_level.items():
        ...
        for isolator_x, isolator_y, isolator_room_key, devices in items:
            ...
            # ветка". На практике сюда почти не должны попадать ветки, у
            # изолятора которых определилось помещение — см. докстринг
            # модуля: BuildSpsSchematic сам приводит помещение ВСЕХ
            # устройств ветки к помещению изолятора ДО вызова этой
            # функции, так что same_room выше почти всегда True; этот
            # путь — запасной, на случай если у изолятора помещение не
            # определилось вовсе.
            room_names = []
            for device, _symbol in valid_devices:
                room_value = get_string_param(device, room_param_name)
