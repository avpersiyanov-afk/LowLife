...
def create_circuit(doc, panel_el, device_els):
    """
    Создаёт электрическую цепь из устройств и подключает её к панели.

    Возвращает (цепь, текст ошибки). Цепь создаётся типом Data — у
    слаботочных шлейфов нет нагрузки, а тип цепи на расчёт длин не влияет.
    """
    ...
    try:
        system = ElectricalSystem.Create(doc, ids, ElectricalSystemType.Data)
    except Exception as ex:
        return None, u"не удалось создать цепь: {}".format(ex)
