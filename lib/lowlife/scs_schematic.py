...
# Насколько глубоко (мм, положительное число) уходит НИЖНЯЯ граница
# самой рамки помещения (блок с иконками устройств) ниже level_y — то же
# число, что -BOTTOM_LINE_MM в sot_schematic.py (там со знаком минус —
# граница ниже level_y; см. _place_room_group, draw_horizontal_line(...,
# current_level_y + BOTTOM_LINE_MM * MM_TO_FT)). Первая линия кабеля
# должна начинаться НЕ от level_y, а от ЭТОЙ границы — иначе линия
# рисуется поверх/внутри самой рамки помещения, а не под ней (ровно
# такой баг уже был: при BOUNDARY_MARGIN_MM=8, отсчитанном от level_y
...
# не несколько разных подобранных на глаз значений: сверху (насколько
# ниже границы рамки помещения проходит коллектор первой, index=0,
# панели), снизу (насколько глубже самой нижней линии кабеля должна
# быть нижняя граница рамки этажа), и по X (насколько правее первой
# линии рамки этажа проходит первый стояк, и насколько левее последнего
# стояка/дорожки — левая граница рамки этажа).
BOUNDARY_MARGIN_MM = 8.0

# X первого (index=0) стояка — на BOUNDARY_MARGIN_MM правее (ближе к
# помещениям) самой левой линии рамки этажа (LEVEL_LINE_1_OFFSET_MM =
# 5мм у СОТ; свой стояк у единственного шкафа СОТ ставится ровно на неё
# же — CABLE_RISER_OFFSET_MM) — не ровно НА этой линии (было так раньше,
# до общего правила "8мм от границы": стояк панели №0 совпадал со
# структурной линией рамки, визуально сливался с ней).
RISER_BASE_OFFSET_MM = 13.0  # = 5 (LEVEL_LINE_1_OFFSET_MM у СОТ) + 8 (BOUNDARY_MARGIN_MM)
...
# Ширина коридора МЕЖДУ ПЕРВОЙ И ВТОРОЙ линией рамки этажа
# (LEVEL_LINE_2_OFFSET_MM у СОТ = 20мм) — сколько места под стояки и
# дорожки магистралей уже есть БЕЗ дополнительного расширения этого
# коридора (см. extra_left_mm у sync_levels/_draw_level_frame,
# BuildScsSchematic.pushbutton — считает по этому числу, на сколько мм
# коридор нужно раздвинуть дальше, если стоякам/дорожкам тут не хватает
# места). Именно ЭТОТ коридор, а не общая ширина рамки из трёх линий —
# коридор ЗА второй линией занят подписью этажа (свой отдельный, третьей
# линией отделённый коридор фиксированной ширины, LEVEL_LINE_3_OFFSET_MM
# = 10мм) и не предназначен для стояков/дорожек СКС.
RISER_CORRIDOR_WIDTH_MM = 20.0
...
def panel_collector_y(panel_index, level_y):
    ...
    level_y — первая панель (index=0) на BOUNDARY_MARGIN_MM ниже НИЖНЕЙ
    ГРАНИЦЫ РАМКИ ПОМЕЩЕНИЯ (level_y - ROOM_BOTTOM_MM, не сразу от
    level_y — иначе линия попадает поверх/внутрь самой рамки помещения),
...
def trunk_lane_x(trunk_index, panel_count):
    ...
    номером (0, 1, 2...) — левее ВСЕХ стояков панелей, на BOUNDARY_MARGIN_MM
    (то же число, что и у остальных границ — не отдельный больший зазор,
    как было раньше) дальше последнего из них — не вплотную (переход к
    дорожке выглядел бы как маленький отросток у стояка панели, а не как
    заметная отдельная линия) и не сразу за пределами уже нарисованного
    коридора рамки этажа (см. RISER_CORRIDOR_WIDTH_MM/extra_left_mm в
    BuildScsSchematic.pushbutton — если стоякам и дорожкам не хватает
    стандартной ширины коридора, он раздвигается). Между собой дорожки
    разных магистралей по-прежнему расставлены с шагом RISER_SPACING_MM.
    Чистая функция, без Revit API.
    """
    last_panel_x = panel_riser_x(panel_count - 1) if panel_count > 0 else 0.0
    return last_panel_x - (BOUNDARY_MARGIN_MM + trunk_index * RISER_SPACING_MM) * MM_TO_FT
