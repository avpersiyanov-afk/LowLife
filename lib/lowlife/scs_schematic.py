...
# Ширина "коридора" из трёх левых линий рамки этажа (LEVEL_LINE_1/2/3_
# OFFSET_MM у СОТ = 5+20+10мм) — сколько места под стояки и дорожки
# магистралей уже есть БЕЗ дополнительного расширения рамки влево (см.
# extra_left_mm у sync_levels/_draw_level_frame,
# BuildScsSchematic.pushbutton — считает по этому числу, на сколько мм
# рамку нужно раздвинуть дальше, если стоякам/дорожкам тут не хватает
# места).
LEVEL_FRAME_WIDTH_MM = 35.0
...
def trunk_lane_x(trunk_index, panel_count):
    ...
    заметная отдельная линия) и не сразу за пределами уже нарисованной
    рамки этажа (см. LEVEL_FRAME_WIDTH_MM/extra_left_mm в
    BuildScsSchematic.pushbutton — если стоякам и дорожкам не хватает
    стандартной ширины рамки, она раздвигается). Между собой дорожки
