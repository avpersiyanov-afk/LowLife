...
# Насколько далеко (в плане, по XY) голова марки может быть от габаритного
# бокса её помещения и всё ещё считаться «сбежавшей за подрезанный контур»
# (не осознанно отведённой выноской). Дальше этого — трогать не будем: это,
# скорее всего, настоящая выноска, специально уведённая в сторону.
_HEAD_ESCAPE_TOL_MM = 1000.0
_HEAD_ESCAPE_TOL_FT = _HEAD_ESCAPE_TOL_MM / MM_IN_FOOT


def _head_near_room_bbox(head, room, transform):
    """
    True, если head (координаты хоста) лежит не дальше _HEAD_ESCAPE_TOL от
    габаритного бокса room в плане (без учёта высоты) — то есть похоже на
    «голову подрезало вместе с контуром», а не на намеренно уведённую
    далеко выноску. False — либо дальше, либо не удалось посчитать (тогда
    её безопаснее не трогать).

    Специально не завязано на RoomTag.HasLeader: у части марок этот флаг
    оказывается True даже когда полки визуально нет (короткая/нулевая
    выноска, дефолт семейства/типа) — расстояние до помещения надёжнее.
    """
    if head is None or transform is None:
        return False
    try:
        local = transform.Inverse.OfPoint(head)
    except Exception:
    ...
    if bbox is None:
        return False
    dx = max(bbox.Min.X - local.X, 0.0, local.X - bbox.Max.X)
    dy = max(bbox.Min.Y - local.Y, 0.0, local.Y - bbox.Max.Y)
