# -*- coding: utf-8 -*-
"""
Чистые (без Revit API) хелперы структурной схемы СКУД: разбивка устройств
контроллера по точкам прохода, сигнатуры состава, подбор типовой группы,
голосование за помещение, раскладочные точки. Оркестрация — в кнопке
BuildSkudSchematic, инкрементальная синхронизация — в skud_schematic_sync.py.

Модель схемы: контроллер — одиночное схемное семейство (категория
«контроллер»), у него нет типового узла-группы; на каждую точку прохода
подбирается группа деталей, чей состав устройств по категориям совпадает с
составом точки прохода. Нет совпадения (no-match) → каждое устройство
отдельным схемным семейством категории, столбиком вниз от точки вставки
точки прохода (device_layout_point).

`lowlife.params` импортируется лениво и с запасным вариантом, поэтому
модуль импортируется и тестируется вне Revit (tests/test_skud_schematic.py).
"""
