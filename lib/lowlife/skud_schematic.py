...
Контроллер — одиночное схемное семейство (категория «контроллер»); у него
нет типового узла-группы. На каждую точку прохода контроллера подбирается
группа деталей, чей состав устройств (по категориям) совпадает с составом
точки прохода. Группа вставляется, разгруппировывается, её схемным
элементам проставляются адрес/марка по сопоставленным реальным устройствам.

Резервный путь (no-match: для состава точки прохода нет подходящей группы)
— каждое устройство отдельным схемным семейством категории, столбиком
вниз от точки вставки точки прохода (device_layout_point).
"""
