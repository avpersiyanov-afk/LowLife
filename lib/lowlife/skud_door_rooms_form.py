# -*- coding: utf-8 -*-
"""
Таблица «помещение -> группа точки доступа» для кнопки «Точки доступа на
двери» (SKUD.panel/PlaceDoorAccessPoints).

Строка на каждое помещение активного вида: номер, имя, уровень, источник
(текущая модель или имя связи), число дверей и выпадающий список групп.
Сверху — поиск по номеру/имени и «Назначить группу всем показанным»
(удобно отфильтровать, например, «серверная», и назначить разом).
Помещения без дверей показаны серыми, группу им выбрать нельзя.
...
NO_GROUP = u"— не расставлять —"

_COLUMNS = [(u"Номер", 80), (u"Имя", None), (u"Уровень", 120), (u"Источник", 170),
            (u"Дверей", 60), (u"Группа точки доступа", 260)]
...
def show_rooms_table(rooms, group_names, saved_choice):
    """
    rooms — список RoomEntry (skud_door_placement), group_names — имена
    модельных групп, saved_choice — {room.key: group_name} с прошлого раза.
    Возвращает {room.key: group_name} (только помещения с выбранной
    группой) либо None, если окно закрыли без «Расставить».
    """
    result = {"choice": None}
    choices = [NO_GROUP] + list(group_names)
    rows = []  # (room, row_grid, combo, search_text)

    win = Window()
    win.Title = u"Точки доступа на двери — выбор групп по помещениям"
    win.Width = 1000
    win.Height = 700
    ...
    hint = TextBlock()
    hint.Text = (
        u"Для каждого помещения выберите группу — из неё берётся только состав "
        u"точки доступа (какие типы семейств), а расставляются элементы на "
        u"каждую дверь помещения по мнемосхеме (Shift+клик по кнопке). "
