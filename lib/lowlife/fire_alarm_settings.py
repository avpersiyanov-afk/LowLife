...
TEXT_FIELDS = [
    ...
    ("room_param_name", u"[Структурная схема] Параметр, в который записываем помещение "
        ...
        u"", False, True),
    ("room_mask", u"[Структурная схема] Маска значения помещения из связи — имена параметров "
        u"Room через запятую и/или скобки: «Имя (Номер)» → «Офис (212)», «Имя, Номер» → "
        u"«Офис, 212». «Имя»/«Номер» — имя и номер помещения. Используется, если параметр "
        u"помещения на устройстве ещё пуст.",
    ...
    ("node_label_offset_mm", u"[Структурная схема] Смещение марки узла вверх от точки вставки, мм",
        u"5", False, True),
    ("same_room_branch_offset_mm", u"[Структурная схема] Ответвление изолятора, чьи устройства "
        u"в том же помещении, что и он сам — на сколько мм ниже ряда изолятора его ставить "
        u"(без отдельной рамки — просто продолжением того же помещения на схеме); пусто или "
        u"0 — по умолчанию (40 мм)",
        u"", False, False),
    ("branch_lot_room_param_name", u"[Структурная схема] Параметр «Имя лота» на ПОМЕЩЕНИИ в "
        u"связанной модели — только для устройств ответвления изолятора: если у помещения, куда "
        u"попадает точка устройства, этот параметр заполнен, его значение принудительно "
        u"записывается в параметр помещения устройства (см. выше) вместо обычного имени/номера "
        u"помещения",
        u"", False, False),
    ("branch_lot_filter_word", u"[Структурная схема] Фильтр «Имя лота» — подстрока (без учёта "
        u"регистра), которая должна встречаться в значении параметра выше, иначе оно "
        u"игнорируется (запись помещения идёт как обычно, без принудительной замены); пусто — "
        u"без фильтра, любое непустое значение считается подходящим. Нужно, если этот параметр "
        u"на помещениях используется не только для лотов, которые должны попасть на схему как "
        u"есть (например «Квартира 12»), но и для более широких группировок (например «МОП»), "
        u"которые на схеме не нужны — впишите общую часть нужных значений, например «квартира»",
        u"", False, False),
    ("max_row_width_mm", u"[Структурная схема] Максимальная ширина строки помещений на этаже, "
        u"мм — следующее помещение, которое уже не помещается в строку, переносится на новую "
        u"строку ниже (как перенос текста по словам, порядок помещений не меняется); пусто или "
        u"0 — не ограничивать (одна строка на этаж, как раньше)",
        u"", False, False),
    ...
    ("schematic_device_categories_text", u"[Структурная схема] Категории устройств схемы (по одной на строку)",
        u"", False, True),
    ("cabinet_category_name", u"[Структурная схема] Категория «Шкаф»/«Панель» — устройство, к "
        u"которому линиями подключаются все остальные (необязательно; должно точно совпадать с "
        u"одним из имён в категориях устройств схемы выше; если пусто — линии не рисуются)",
        u"", False, False),
    ("draw_loop_lines", u"[Структурная схема] Рисовать кольцевые шлейфы и ответвления изоляторов "
        u"(линии) — впишите «нет», чтобы построить только раскладку узлов по этажам/помещениям, "
        u"без линий (заметно быстрее на больших моделях; линии можно добавить следующим запуском, "
        u"когда раскладка устроит)",
        u"да", False, False),
    ("draw_tags", u"[Структурная схема] Ставить марки узлов (IndependentTag) — впишите «нет», "
        u"чтобы построить раскладку без марок: сама вставка марки — штатный вызов Revit API, "
        u"который на некоторых моделях занимает секунду и больше НА КАЖДУЮ марку и легко "
        u"становится основным временем построения схемы при сотнях устройств. Марки можно "
        u"добавить следующим запуском, когда раскладка устроит",
        u"да", False, False),
]
