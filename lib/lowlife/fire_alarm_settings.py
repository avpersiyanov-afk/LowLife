...
TEXT_FIELDS = [
    ...
    ("same_room_branch_offset_mm", u"[Структурная схема] Ответвление изолятора, чьи устройства "
        ...
        u"", False, False),
    ("branch_lot_param_name", u"[Структурная схема] Параметр «Имя лота» на устройстве (например "
        u"SMNX_Имя лота) — для рамки-спутника ответвления изолятора из РАЗНЫХ помещений: если "
        u"это значение заполнено и одинаково у ВСЕХ устройств ветки, подпись рамки берётся из "
        u"него, а не из перечисления через «; » их (разных) помещений; необязательно — если "
        u"пусто, или значение не общее для всей ветки, подпись как раньше — из помещений",
        u"", False, False),
