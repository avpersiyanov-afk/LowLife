...
SETTINGS_FILE_NAME = "LowLifeRoomLots_settings.json"
...
def require(settings, keys):
    ...
    if missing:
        forms.alert(
            ...
            u"Откройте настройки: Shift+клик по кнопке «Двухуровневые лоты».".format(
                u"\n".join(missing)
            ),
            exitscript=True
        )
...
def show_settings_form(values):
    ...
    win = Window()
    win.Title = u"Настройки: Двухуровневые лоты"
    win.Width = 720
    win.Height = 440
    ...
    title = TextBlock()
    title.Text = u"Параметры имени лота и номера секции"
    title.FontSize = 16
    ...
    settings_transfer.add_transfer_buttons(
        buttons, _read_all, _write_all, u"двухуровневых лотов", _on_settings_imported
    )
...
def get_settings_interactive():
    ...
    по Shift+клику на кнопке «Двухуровневые лоты».
    """
    while True:
...
def get_settings_silent():
    ...
    строка, если ещё не настроено). Используется кнопкой «Двухуровневые лоты»."""
