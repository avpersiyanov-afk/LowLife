...
категорий активного вида (с числом элементов в каждой), отмеченное
сохраняется на следующий обычный клик. Тот же приём Shift+клика, что у
...
def configure(doc, view):
    """
    Shift+клик: чек-лист категорий активного вида (с числом элементов в
    каждой), предвыбор — то, что отмечено сейчас. Сохраняет отметку в
    файл настроек; отмена (Esc/крестик) ничего не меняет.
    """
