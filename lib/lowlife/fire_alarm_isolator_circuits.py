...
- Все выбранные устройства ручного пуска (ИПР, «ручной» в имени типа) —
  одна общая цепь (в одной цепи может быть несколько ИПР).
- Остальные выбранные устройства — вторая общая цепь.
- Итого у изолятора максимум MAX_CIRCUITS_PER_ISOLATOR цепи («ручная» и
  «автоматическая»); при превышении ничего не создаётся, кнопка сообщает
  об ошибке (жёсткий лимит, без частичного создания).
...
для линий проводки, собственным именем Revit цепи (circuit.Name,
например «Цепь 1») подписываются уже нарисованные вручную линии рядом с
устройствами/изолятором цепи (см. lowlife.fire_alarm_wire_marks.mark_wire_lines)
— у этой кнопки нет «Номера цепи», как у build_loop_circuits (там на
линию идёт именно он), поэтому подпись — имя элемента цепи в Revit.
"""
...
def split_manual_devices(device_els):
    ...
    Обе группы дальше идут каждая в ОДНУ цепь целиком (несколько ИПР в
    одной цепи — норма для реальных проектов), а не по цепи на устройство.
    """
...
def build_isolator_device_circuits(doc, device_els, isolator_el, settings):
    ...
    # Каждая группа -> ровно одна цепь: все ИПР вместе в «ручную» цепь,
    # все остальные устройства — в «автоматическую». Не по цепи на
    # устройство: в одной цепи изолятора бывает несколько ручных
    # извещателей.
    groups = []
    if manual_devices:
        groups.append(manual_devices)
    if other_devices:
    ...
    with revit.Transaction(u"Цепи изолятор-устройства СПС"):
        for group in groups:
            ...
            # устройствами/изолятором цепи — собственным именем Revit цепи
            # (circuit.Name, например «Цепь 1»): у «изолятор-устройства»
            # нет «Номера цепи», в отличие от магистральных шлейфов, там
            # на линию идёт именно он (см. calc_loop_lengths).
