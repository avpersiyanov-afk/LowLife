...
**Выключен по умолчанию** (``export_rename`` DEFAULTS ``watch_explorer =
False``) — ``install()`` первым делом читает настройки и, если авто-режим
выключен, не подписывается вообще ни на что; работает только кнопка
«Переименование выгрузки» вручную. Включить — Shift+клик по кнопке.
