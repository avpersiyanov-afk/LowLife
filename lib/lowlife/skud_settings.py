...
TEXT_FIELDS = [
    ...
    ("schematic_device_categories_text", u"[Схема] Категории устройств схемы (по одной на строку)",
        u"", False, True, True),
    ("passage_point_param", u"[Схема] Параметр устройства «Номер точки прохода» (пусто у устройства = точка прохода 1)",
        u"", False, False, False),
    ("schematic_view_name", u"[Схема] Имя чертёжного вида структурной схемы (создаётся с этим именем; "
        u"при повторных запусках обновляется вид с этим именем)",
    ...
    ("manifest_param_name", u"[Схема] Служебный параметр вида для хранения манифеста схемы "
        ...
        u"", False, True, False),
    ("schematic_source_uid_param", u"[Схема] Служебный параметр схемного семейства для UniqueId "
        u"исходного устройства (текстовый, привязан к категории схемных семейств — нужен, чтобы "
        u"при повторном запуске узнавать «то же устройство»)",
        u"", False, True, False),
]
