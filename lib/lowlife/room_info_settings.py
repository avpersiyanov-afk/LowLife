...
TEXT_FIELDS = [
    (
        ...
        u"Параметр элемента",
        u"Введи имя параметра для записи имени помещения в твоих "
        u"семействах. Это должен быть общий параметр. Например "
        u"«SMNX_Помещение».",
        u"", True
    ),
    (
        ...
        u"Имена параметров помещения (Room) в связи, разделённые запятой "
        u"и/или скобками. «Имя, Номер» → «Офис, 212»; «Имя (Номер)» → "
        u"«Офис (212)». «Имя»/«Номер» — имя и номер помещения; прочие "
        u"токены ищутся как параметры Room. Токен без значения выпадает "
