...
| `trunk_jump_geometry` | `trunk_jump_geometry(min_y_a, max_y_a, min_y_b, max_y_b)` | Геометрия перехода между двумя стояками: если Y-диапазоны пересекаются — переход на верхней границе пересечения, без достройки; если нет — переход ПОСЕРЕДИНЕ разрыва, оба стояка достраиваются коротким продолжением до неё (не один стояк на всю длину разрыва — у панелей на далёких друг от друга этажах разрыв может быть длиной во много этажей, отрезок вышел бы аномально длинным). Возвращает `(jump_y, ext_a, ext_b)` — `ext_*` это `None` либо `(from_y, to_y)`. Чистая функция без Revit API |
| `sync_trunk_links` | `sync_trunk_links(doc, view, new_state, old_trunk_line_ids, trunk_links, riser_info)` | Магистральная связь шкаф-шкаф (`trunk_links` — `[(panel_uid_a, panel_uid_b), ...]`, из `scs.collect_target_panel_devices`) ведётся через уже нарисованные стояки панелей (`riser_info` — второй элемент возврата `sync_panel_buses`) — горизонтальный переход между ними по `trunk_jump_geometry`, а не прямой линией через рамки помещений. Общий фиксированный **яркий и жирный** стиль (`TRUNK_COLOR_RGB` — ярко-красный, не серый: серый на сложной чёрно-белой схеме легко теряется на глаз; `TRUNK_LINE_WEIGHT` — жирная линия), не цвет какой-то одной панели. Тоже не диффится — удаляется и рисуется заново каждый запуск. Возвращает `(line_ids, skipped)` — `line_ids` для `state["trunk_line_ids"]`; `skipped` — `[(panel_uid_a, panel_uid_b, reason), ...]` для пар, где не нарисовано ни одного отрезка (`reason` — `"no_riser_a"`/`"no_riser_b"` (у панели нет записи в `riser_info` — не размещена на схеме/шина не нарисована) или `"draw_failed"` (стояки есть, но `draw_segment` не смог построить ни один отрезок)) — диагностика для отчёта кнопки |
