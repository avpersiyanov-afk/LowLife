...
дальше выбирается одна ИЛИ несколько «базовых» линий (остаются целыми),
затем одна ИЛИ несколько «вторых» линий за один `PickObjects` — каждая из
них разрывается там, где пересекает базовые, по правилам выбранного
режима, без повторного запроса диаметра/режима.
...
Оба режима — ОДНА функция (`plan_bypass_for_line`): различие только в
`group_crossing_positions` (группирует найденные пересечения либо в один
общий "пролёт", либо по одному пролёту на пересечение), дальше построение
одинаковое. При ровно одном пересечении оба режима дают одинаковый
результат. Диаметр — буквально диаметр дуги для одиночного пролёта: на
второй линии вырезается прямой участок длиной ровно `diameter`, а дуга —
ровно полуокружность этого диаметра (обе точки разрыва и выбранная
«верхняя» точка обхода равноудалены от центра пролёта на `radius_ft`,
поэтому `Arc.Create(c1, c2, bulge_point)` с тремя РАЗНЫМИ точками всегда
даёт открытую полуокружность, а не замкнутую окружность); при нескольких
...
Сторона обхода не выводится жёстким правилом — пользователь указывает её
сам кликом (`PickPoint`) отдельно для каждого пролёта, т.к. заранее
...
(`highlight_element`, `uidoc.Selection.SetElementIds`) — иначе при обходе
нескольких линий/пролётов подряд легко потерять, к какой из них относится
текущий запрос стороны.
...
оригинала — тот же принцип delete+recreate вместо правки существующего
элемента, что и у марок узлов СКС/СОТ (см. `scs_schematic.py` ниже), т.к.
поведение сеттера `GeometryCurve` негде проверить вживую.
...
| `highlight_element` | `highlight_element(uidoc, el)` | Выделяет один элемент в модели (`SetElementIds`) — перед `PickPoint` за сторону обхода |
...
| `plan_bypass_for_line` | `plan_bypass_for_line(uidoc, base_lines, second_el, radius_ft, bundle_mode)` | `base_lines` — список `(p1, p2)` концов каждой базовой линии. Строит пролёты (см. выше), для каждого подсвечивает вторую линию и спрашивает (`PickPoint`) сторону обхода; `(plan, error)` — `plan` — `{"element": second_el, "curves": [Line/Arc, ...]}` для `apply_bypass_plan`, либо `None` с причиной пропуска (нет пересечений, диаметр/запас не помещается, пролёты пересекаются между собой в «Одиночном» режиме, отмена выбора стороны) |
| `apply_bypass_plan` | `apply_bypass_plan(doc, view, plan)` | Строит обход по `plan`: удаляет исходную вторую линию, создаёт `plan["curves"]` новыми `DetailCurve` со стилем линии оригинала |
| `run_line_bypass_tool` | `run_line_bypass_tool(doc, uidoc, view)` | Весь сценарий кнопки: диаметр → режим → базовая(-ые) линия(-и) → вторая(-ые) линия(-и) → обходы |
