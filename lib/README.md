...
2r, чтобы садиться в сёдла). Если центрированный треугольник упирается в верх
лотка (много одинаковых кабелей/ромашек) — `_pyramid_offsets` оставляет
пирамидку как есть, а лишние круги кладёт отдельным столбиком вплотную к
боковой стенке лотка (пирамидка сдвигается вправо на его ширину). `ties` из `plan_section` — кольца-стяжек,
