...
| `trunk_component_segments` | `trunk_component_segments(member_points, lane_x)` | Отрезки магистральной цепочки: короткий горизонтальный отвод от каждого стояка цепочки (`member_points` — `[(x, y), ...]`, по одному на панель-члена) до общей дорожки (X=`lane_x`), плюс ОДИН общий вертикальный участок дорожки, покрывающий весь диапазон Y цепочки (только если членов >= 2 и их Y различаются). Вырожденные (нулевой длины) отводы (`x == lane_x`) не включаются. Возвращает `[(x1,y1,x2,y2), ...]`. Чистая функция без Revit API |
| `sync_trunk_links` | `sync_trunk_links(doc, view, new_state, old_trunk_line_ids, trunk_links, riser_info, panel_count)` | Магистральные связи шкаф-шкаф (`trunk_links` — `[(panel_uid_a, panel_uid_b), ...]`, из `scs.collect_target_panel_devices`) группируются в цепочки (`group_trunk_components`) и каждая цепочка ведётся по своей ОДНОЙ общей дорожке (`trunk_lane_x`/`trunk_component_segments`), не по X одного из стояков панелей (была так раньше — магистраль визуально совпадала с обычной шиной той панели) и не по отдельной дорожке на каждую пару связей (давало "боксовый"/ступенчатый вид при цепочках из 3+ панелей) — подключение к каждому стояку по его верхней точке (`max_y` из `riser_info`, второй элемент возврата `sync_panel_buses`; панель без записи в `riser_info` просто исключается из отводов своей цепочки, на остальных членах это не сказывается). Общий фиксированный **яркий и жирный** стиль (`TRUNK_COLOR_RGB` — ярко-красный, не серый: серый на сложной чёрно-белой схеме легко теряется на глаз; `TRUNK_LINE_WEIGHT` — жирная линия), не цвет какой-то одной панели. Тоже не диффится — удаляется и рисуется заново каждый запуск. Возвращает `(line_ids, skipped)` — `line_ids` для `state["trunk_line_ids"]`; `skipped` — `[(panel_uid_a, panel_uid_b, reason), ...]` для исходных пар `trunk_links`, у которых хотя бы одна панель не размещена на схеме (`reason` — `"no_riser_a"`/`"no_riser_b"`) — диагностика для отчёта кнопки |
