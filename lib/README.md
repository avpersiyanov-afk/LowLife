...
| `trunk_lane_x` | `trunk_lane_x(trunk_index, panel_count)` | X отдельной вертикальной "дорожки" магистрали с этим номером — продолжение той же последовательности, что и `panel_riser_x` (дорожка магистрали 0 = "стояк панели номер panel_count"), поэтому никогда не совпадает по X ни с одним стояком панели. Чистая функция без Revit API |
