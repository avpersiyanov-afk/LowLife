...
| `merge_nodes` | `merge_nodes(nodes, tol, points_close_fn)` | Объединяет узлы трассы ближе `tol` друг к другу в один, суммируя `categories`, `segment_ids`, `device`; `points_close_fn` обычно — `geometry.points_close`. Итоговая точка кластера выбирается **детерминированно** (не зависит от порядка обхода узлов, который может отличаться между запусками): точка помеченного узла (панель/стояк), если такая есть в кластере, иначе — точка с наименьшими координатами (X, Y, Z). Раньше бралась точка первого добавленного в кластер узла — из-за нестабильного порядка обхода `dict` она могла на пару мм смещаться между запусками и пересекать границу допуска дедупа в PlaceRouteNodes, создавая копию элемента вместо обновления существующего |
