...
2r, чтобы садиться в сёдла). Если центрированный треугольник не влезает по
высоте (много одинаковых кабелей/ромашек) — `_pyramid_offsets` переходит на
плотную укладку от левой стенки: ряды не сужаются ниже, чем нужно для
остатка, лишние круги идут добавочными рядами со стороны стенки. `ties` из `plan_section` — кольца-стяжек,
