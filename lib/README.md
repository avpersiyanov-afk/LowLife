...
- Общая 3D-геометрия (используется и для помещений, и для препятствий):
  ...
  (для призмы это и есть контур в плане — годится и для помещения, и для
  колонны постоянного сечения).
- Препятствия внутри помещения (колонны и т.п.), независимо от обрезки по
  помещению: `_prepare_obstacle_sources(doc, cat_ids)` — один раз за
  прогон кнопки собирает элементы `obstacle_categories` (по умолчанию
  `OST_Columns, OST_StructuralColumns`) из текущей модели и **всех**
  связей (колонны обычно в отдельной конструктивной связи, не в той, где
  комнаты) с их bbox; `_gather_obstacle_rings(...)` на камеру — быстрая
  отбраковка по высоте (габарит элемента должен захватывать плоскость
  расчёта) и расстоянию, затем `_footprint_rings` (реальный контур, не
  bounding box), с кэшем по элементу (`ring_cache`, общий на весь прогон
  — геометрия не зависит от камеры). Возвращает `(rings, note)`: перед
  подмешиванием к контуру помещения как ещё одних «дыр» отсеивает два
  типовых источника «пустого контура» — препятствие в пределах
  `_OBSTACLE_SELF_CLEARANCE_FT` (~300 мм) от самой камеры (камера
  смонтирована на этом же препятствии — оно не может загораживать само
  себя) и препятствие с площадью контура больше `_OBSTACLE_MAX_AREA_FT2`
  (~18.6 м², крупнее любой реальной колонны — обычно значит, что Revit
  «сшил» (joined) тело колонны с соседним перекрытием/стеной, и «нижняя
  грань» получившегося Solid — это контур перекрытия, накрывающий саму
  камеру). `note` — что именно и сколько отсеяно, уходит в отчёт.
- `_zone_curveloop` строит звёздный многоугольник от точки камеры (или
  кольцевой сектор при мёртвой зоне), обрезая **каждый луч** по
  `_clip_distance` по ВСЕМ переданным контурам (помещение + препятствия
  одинаково) — работает с вогнутыми помещениями, «дырами» и тенями от
  препятствий сразу. Соседние точки контура склеиваются порогом
  ...
  вершин (сложная граница помещения, много препятствий рядом) один такой
  отрезок обрывал построение всего контура; теперь проблемный отрезок
  пропускается, не отменяя остальное. Если совместная обрезка всё равно
  выродила контур — `_one_camera` пробует прогрессивно ослабить её (без препятствий, затем
  вовсе без обрезки) прежде чем сдаться.
