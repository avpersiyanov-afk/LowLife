...
Сценарий: диаметр запрашивается один раз в начале запуска, дальше
выбирается одна ИЛИ несколько «базовых» линий (остаются целыми), затем
одна ИЛИ несколько «вторых» линий за один `PickObjects` — каждая из них
разрывается там, где пересекает базовые, и разрыв соединяется дугой уже
известного диаметра, без повторного запроса. Если вторая линия пересекает
сразу НЕСКОЛЬКО базовых (типовой случай — одна линия обходит пучок
соседних линий), вместо N мелких дуг по одной на каждое пересечение
строится ОДНА большая дуга на все эти пересечения — вырезаемый участок
растягивается от первого до последнего пересечения (по расстоянию вдоль
второй линии) с запасом `radius_ft` по каждому краю; при ровно одном
пересечении формула вырождается в старый случай (единственная точка — она
же и минимум, и максимум), поэтому это одна функция без раздельных веток
"малая дуга"/"большая дуга". Диаметр — буквально диаметр дуги для случая
одного пересечения: на второй линии вырезается прямой участок длиной ровно
`diameter`, а дуга — ровно полуокружность этого диаметра (обе точки
разрыва и выбранная «верхняя» точка обхода равноудалены от центра дуги на
`radius_ft`, поэтому `Arc.Create` всегда даёт полуокружность); при
нескольких пересечениях центр — уже середина между точками разрыва, не
сама точка пересечения (их несколько), принцип тот же, хорда просто
длиннее.

Сторона обхода не выводится жёстким правилом — пользователь указывает её
сам кликом (`PickPoint`) отдельно для каждой второй линии, т.к. заранее
...
нескольких линий подряд легко потерять, к какой из них относится текущий
запрос стороны.

Оригинальная вторая линия не правится на месте (не трогается
`GeometryCurve`) — удаляется и на её месте создаются три новых
`DetailCurve` (прямая-дуга-прямая) со стилем линии оригинала — тот же
принцип delete+recreate вместо правки существующего элемента, что и у
марок узлов СКС/СОТ (см. `scs_schematic.py` ниже), т.к. поведение
сеттера `GeometryCurve` негде проверить вживую.

| Функция | Сигнатура | Что делает |
|---|---|---|
| `ask_diameter_mm` | `ask_diameter_mm()` | Диалог ввода диаметра дуги, мм (при обходе нескольких линий сразу — запас по краям общей дуги); `None`, если отменено |
...
| `plan_bypass` | `plan_bypass(uidoc, base_lines, second_el, radius_ft)` | `base_lines` — список `(p1, p2)` концов каждой базовой линии. Считает ВСЕ пересечения второй линии с `base_lines`, растягивает вырезаемый участок от первого до последнего (плюс запас), подсвечивает линию и спрашивает (`PickPoint`) сторону обхода; `(plan, error)` — `plan` для `apply_bypass_plan`, либо `None` с причиной пропуска (нет пересечений, диаметр/запас не помещается, отмена выбора стороны) |
| `apply_bypass_plan` | `apply_bypass_plan(doc, view, plan)` | Строит обход по `plan`: удаляет исходную вторую линию, создаёт три новых `DetailCurve` со стилем линии оригинала |
| `run_line_bypass_tool` | `run_line_bypass_tool(doc, uidoc, view)` | Весь сценарий кнопки: диаметр → базовая(-ые) линия(-и) → вторая(-ые) линия(-и) → обходы |
