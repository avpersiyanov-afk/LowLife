...
схеме. Топология сделана по требованию пользователя НЕ как "N
независимых шин" (`sync_cable_connections` СОТ, размноженная на панель),
а как **общий коллектор + отдельные цветные стояки**: на этаже один
общий горизонтальный коллектор (все устройства этажа — общие отростки
на одну линию, независимо от их панели — имитирует физику: на этаже
кабели разных панелей часто идут в одном лотке), расходится только в
стояке — там у каждой панели своя отдельная вертикальная линия.
Коллектор конкретного этажа дотягивается по X только до стояков тех
панелей, у которых есть устройство на этом этаже. Раз коллектор общий,
отросток одного устройства визуально не отличить от отростка другого
без доп. признака — поэтому отростки и стояк каждой панели красятся в
свой автоматически создаваемый Line Style (без настроек, по имени
панели); сам коллектор остаётся нейтральным.
...
| `sync_shared_bus` | `sync_shared_bus(doc, view, new_state, old_bus_line_ids, panels_order, panel_device_uids, panel_names, drop_offset_mm=BUS_DROP_OFFSET_MM)` | Один коллектор на этаж (все устройства этажа — общие отростки), дотянутый по X до стояка каждой панели, у которой есть устройство на этом этаже (`panel_device_uids[panel_uid]`, должен включать и сам `panel_uid`); стояк каждой панели (свой X — `panel_riser_x(index)`) идёт по Y только через те этажи, где у неё реально есть устройства. `panel_names` — `{panel_uid: имя}`, для подписи Line Style. Как и у СОТ (`sync_cable_connections`), эти линии не диффятся — полностью удаляются и рисуются заново на каждом запуске (выводятся из уже посчитанных `sync_levels` позиций, диффить незачем). Возвращает плоский `[line_id, ...]` для `state["bus_line_ids"]` |
...
физически за пределами уже нарисованной рамки этажа) и
