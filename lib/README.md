...
Единственное, чего нет у СОТ и что есть только здесь — несколько панелей
на одной схеме. Топология сделана по требованию пользователя НЕ как "N
независимых шин" (`sync_cable_connections` СОТ, размноженная на панель),
а как **общий коллектор + отдельные стояки**: на этаже один общий
горизонтальный коллектор (все устройства этажа — общие отростки на одну
линию, независимо от их панели — имитирует физику: на этаже кабели
разных панелей часто идут в одном лотке), расходится только в стояке —
там у каждой панели своя отдельная вертикальная линия. Коллектор
конкретного этажа дотягивается по X только до стояков тех панелей, у
которых есть устройство на этом этаже.
...
| `sync_shared_bus` | `sync_shared_bus(doc, view, new_state, old_bus_line_ids, panels_order, panel_device_uids, drop_offset_mm=BUS_DROP_OFFSET_MM)` | Один коллектор на этаж (все устройства этажа — общие отростки), дотянутый по X до стояка каждой панели, у которой есть устройство на этом этаже (`panel_device_uids[panel_uid]`, должен включать и сам `panel_uid`); стояк каждой панели (свой X — `panel_riser_x(index)`) идёт по Y только через те этажи, где у неё реально есть устройства. Как и у СОТ (`sync_cable_connections`), эти линии не диффятся — полностью удаляются и рисуются заново на каждом запуске (выводятся из уже посчитанных `sync_levels` позиций, диффить незачем). Возвращает плоский `[line_id, ...]` для `state["bus_line_ids"]` |

`RISER_BASE_OFFSET_MM` (45мм — первый стояк ставится за левыми линиями
самой рамки этажа, `LEVEL_LINE_1/2/3_OFFSET_MM` = 5+20+10 = 35мм у СОТ),
`RISER_SPACING_MM` (15мм между стояками разных панелей — масштаб схемы,
не 300мм, как было в первой версии: с шагом 300мм при масштабе схемы
"20мм между устройствами" коллектор до дальнего стояка тянулся через
явно пустой промежуток) и `BUS_DROP_OFFSET_MM` (тот же смысл, что
`CABLE_DROP_OFFSET_MM` у СОТ) —
