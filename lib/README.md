...
- Обрезка по помещению из связи: `room_rings_for_point(doc, точка_с_Z,
  ...
  XY выбрано). `_zone_curveloop` строит звёздный многоугольник от точки
  камеры (или кольцевой сектор при мёртвой зоне), обрезая **каждый луч**
  по `_clip_distance` — работает с вогнутыми помещениями и «дырами».
