...
Сценарий: диаметр и режим обхода запрашиваются один раз в начале запуска.
Базовые линии выбираются ОДНИМ `PickObjects` (многократный выбор — рамкой
и/или кликами, Enter/«Готово» подтверждает один раз) — многократный выбор
специально сохранён именно на этом шаге. Дальше — цикл (`run_line_bypass_tool`):
`PickObject` на ОДНУ вторую линию → сразу `PickPoint` на сторону обхода
именно для неё → клик стороны сразу строит и коммитит обход этой линии
(без предпросмотра/подтверждения) → цикл возвращается к выбору следующей
второй линии. Esc на шаге выбора второй линии заканчивает всю кнопку —
единственная явная точка выхода; Esc на шаге выбора стороны пропускает
только текущую линию, не завершая работу кнопки.

Честно про "живой предпросмотр, следующий за мышью": `uidoc.Selection.PickPoint`
в Revit API — блокирующий вызов без обратных вызовов на движение мыши, он
физически не даёт скрипту перерисовывать свою графику вслед за курсором
до клика — это ограничение платформы для обычных `IExternalCommand`, а не
недоделанная часть этого файла. Настоящий живой предпросмотр потребовал бы
совсем другого класса решения (свой `IDirectContext3DServer` и/или ловля
курсора через `Idling` с ручным пересчётом экран→модель, по сути отдельный
модлесс-инструмент) — непроверенная здесь механика, которую не стали
городить без живого Revit под рукой (см. CLAUDE.md). Вместо этого клик по
стороне сразу и строит, и коммитит дугу.
...
- «Одиночный» (`bundle_mode=False`) — если вторая линия пересекает
  ...
  линии, если пересечений несколько).
- «Множественный» (`bundle_mode=True`) — все пересечения этой второй
  линии с базовыми объединяются в ОДНУ большую дугу на весь пучок:
  вырезаемый участок растягивается от первого до последнего пересечения
  (по расстоянию вдоль второй линии) с запасом `radius_ft` по каждому
  краю — тем же diameter, что был бы диаметром дуги при одиночном
  пересечении.

Оба режима считаются ОДНОЙ и той же геометрией (`compute_line_plan` +
`build_curves_for_side`): различие только в `group_crossing_positions`
(группирует найденные пересечения либо в один общий "пролёт", либо по
одному пролёту на пересечение), дальше построение одинаковое. При ровно
одном пересечении оба режима дают одинаковый результат. Диаметр —
буквально диаметр дуги для одиночного пролёта: на второй линии вырезается
прямой участок длиной ровно `diameter`, а дуга — ровно полуокружность
этого диаметра (обе точки разрыва и выбранная «верхняя» точка обхода
равноудалены от центра пролёта на `radius_ft`, поэтому
`Arc.Create(c1, c2, bulge_point)` с тремя РАЗНЫМИ точками всегда даёт
открытую полуокружность, а не замкнутую окружность); при нескольких
пересечениях в «Множественном» режиме центр — уже середина между точками
разрыва, принцип и гарантия та же, хорда просто длиннее. В «Одиночном»
режиме соседние пролёты не должны накладываться — если для заданного
диаметра два пересечения слишком близко, вся вторая линия пропускается с
ошибкой, предлагающей уменьшить диаметр или переключиться на
«Множественный», а не тихо слипается в одну дугу.

Оригинальная вторая линия не правится на месте (не трогается
`GeometryCurve`) — удаляется и на её месте создаются новые `DetailCurve`
(прямая-дуга-прямая-дуга-...-прямая, по числу пролётов) со стилем линии
оригинала — тот же принцип delete+recreate, что и у марок узлов СКС/СОТ
(см. `scs_schematic.py` ниже). Каждая линия коммитится своей отдельной
транзакцией сразу после клика стороны (`build_and_commit_line`) — раньше
здесь пробовали держать одну общую транзакцию открытой поперёк нескольких
`PickPoint`/`forms.alert` ради предпросмотра с возможностью откатить и
перевыбрать сторону; от этого отказались в пользу текущего более простого
цикла «выбрал — сразу построилось».

| Функция | Сигнатура | Что делает |
|---|---|---|
| `ask_diameter_mm` | `ask_diameter_mm()` | Диалог ввода диаметра дуги, мм (при «Множественном» обходе — запас по краям общей дуги); `None`, если отменено |
| `ask_bypass_mode` | `ask_bypass_mode()` | Диалог выбора режима (см. выше); `True` — «Множественный», `False` — «Одиночный» (в т.ч. если окно закрыли без ответа — безопасный вариант по умолчанию) |
| `segment_intersection_xy` | `segment_intersection_xy(p1, p2, p3, p4, tol=1e-9)` | Точка пересечения отрезков `(p1-p2)` и `(p3-p4)` в плоскости XY; `None`, если параллельны/коллинеарны или пересекаются только на продолжении |
| `highlight_elements` | `highlight_elements(uidoc, elements)` | Выделяет элементы в модели (`SetElementIds`) — перед `ask_side_reference`, чтобы было видно, о какой линии речь |
| `ask_side_reference` | `ask_side_reference(uidoc, prompt)` | Один `PickPoint` — точка, определяющая сторону обхода для текущей линии; `None`, если отменено (Esc) |
| `collect_crossing_positions` | `collect_crossing_positions(a1, a2, dir_x, dir_y, base_lines)` | Расстояния (от `a1`, вдоль направления второй линии) всех точек пересечения `(a1-a2)` с любой из `base_lines` |
| `group_crossing_positions` | `group_crossing_positions(positions, bundle_mode)` | Группирует позиции в список "пролётов" — один на все позиции (`bundle_mode=True`) либо по одному на позицию (`bundle_mode=False`) |
| `compute_line_plan` | `compute_line_plan(base_lines, second_el, radius_ft, bundle_mode)` | Считает пролёты второй линии (без выбора стороны); `(line_plan, error)` — `line_plan` — геометрия, достаточная для `build_curves_for_side`, либо `None` с причиной пропуска (нет пересечений, диаметр/запас не помещается, пролёты пересекаются в «Одиночном» режиме) |
| `build_curves_for_side` | `build_curves_for_side(line_plan, side_pt)` | Строит список `Line`/`Arc` (прямая-дуга-...-прямая) для `line_plan` под точку стороны `side_pt` |
| `build_and_commit_line` | `build_and_commit_line(doc, view, line_plan, side_pt)` | `build_curves_for_side` + сразу коммит: удаляет исходную вторую линию, создаёт новые `DetailCurve` со стилем оригинала, всё в одной транзакции |
| `run_line_bypass_tool` | `run_line_bypass_tool(doc, uidoc, view)` | Весь сценарий кнопки: диаметр → режим → базовые линии (`PickObjects`) → цикл «вторая линия → сторона → сразу построить», пока не нажмут Esc на выборе линии |
...
блоб макросов, парсеру не нужный, поэтому `.xlsm` читается тем же кодом);
работаем через `System.IO.Compression.ZipArchive` на IronPython 2. Запись — один лист, без
...
Раздел «Типовые группы точек прохода» окна: мультивыбор групп деталей
(по одной на типовой состав точки прохода) — источник
`scs_settings.list_detail_group_types` (только `OST_IOSDetailGroups`),
обёртка `GroupTypeOption`; прямой ключ JSON `passage_point_group_ids`
(список), с `load_*`/`save_*`. У контроллера типового узла-группы нет —
его схемное семейство берётся из таблицы категорий по строке «контроллер»
...
## skud_schematic.py
Логика кнопки **BuildSkudSchematic** («Структурная схема»): контроллер —
одиночное схемное семейство (категория «контроллер»), а на каждую его
...
Группы точек прохода выбираются в окне «Параметры СКУД» → «Типовые группы
точек прохода» (только группы деталей, `scs_settings.list_detail_group_types`);
ключ JSON `passage_point_group_ids`. Схемное семейство контроллера — из
таблицы категорий (строка «контроллер»).
...
диагностика: узнать точный id кнопки «Экспорт» ModPlus, раз журналы Revit
не пишутся. `arm()` (обычный клик) подписывается на
`Autodesk.Windows.ComponentManager.ItemExecuted` (`clr.AddReference
("AdWindows")`) — статическое событие, срабатывает на клик по любой кнопке
ленты, в т.ч. сторонних плагинов, даёт `RibbonItem` с `Id`/`Text`/`Cookie`.
Каждый пойманный элемент дописывается строкой в
`%APPDATA%\pyRevit\LowLifeExportRename_commands.log` после строки-маркера
`=== armed <ts>`; `show_results()` (Shift+клик) показывает всё после
последнего маркера и, если в строке есть `modplus`/`экспорт`, кладёт её
`Id` в `export_rename` `last_seen_command`. Подписка на статическое
событие переживает перезапуск движка pyRevit между нажатиями (ценой
возможной висящей подписки до перезапуска Revit — для разовой диагностики
ок).

## rename_by_list.py
Тело кнопки `ToolsExport.panel/RenameByList` («Переименование по списку»).
Универсальный ручной парный переименователь — в отличие от `export_rename.py`
(фиксированная замена одной подстроки), здесь список пар «старое → новое
имя» произвольный и его составляет пользователь в таблице.

Сценарий: выбранная папка должна содержать две подпапки первого уровня
(если их больше двух — `forms.SelectFromList` с multiselect просит выбрать
ровно две). Из файлов этих двух подпапок собирается объединённое множество
имён БЕЗ РАСШИРЕНИЯ (`_stems`) — типовой случай: DWG- и PDF-выгрузка с
одинаковыми номерами листов в разных подпапках. Список показывается в
WPF `DataGrid` (тот же приём, что `family_catalog.show_status_form`):
левый столбец — текущее имя, только для чтения; правый — редактируемый
(`Binding` с `Mode=TwoWay`, `UpdateSourceTrigger=PropertyChanged`),
изначально равен левому. ОК проверяет новые имена на недопустимые символы
файловой системы (`INVALID_FILENAME_CHARS`) и возвращает только
ИЗМЕНЁННЫЕ строки как `{старое: новое}`.

`plan_renames(root, mapping)` затем обходит **всю** выбранную корневую
папку рекурсивно (`os.walk`, не только те две подпапки, откуда собирался
список) — каждый файл, чьё имя без расширения совпадает с ключом mapping,
планируется к переименованию с сохранением расширения; коллизия (целевое
имя уже занято) — статус `"collision"`, пропускается. В модели Revit
ничего не меняется, транзакция не нужна.

| Функция | Сигнатура | Что делает |
|---|---|---|
| `collect_names` | `collect_names(root)` | `(stems, folder_a, folder_b)` — имена без расширения из двух подпапок первого уровня, либо `None` (с `alert`) |
| `plan_renames` | `plan_renames(root, mapping)` | `[(src, dst, status)]` по всей папке `root` рекурсивно, `status` `"ok"`/`"collision"` |
| `apply_renames` | `apply_renames(plans)` | Переименовывает пары со статусом `"ok"`; `(renamed, errors)` |
...
- `_find_param(doc, el, name)` — параметр камеры по имени: сначала
  ...
  видит в принципе), и только если там нет значения — тот же параметр у
  типа (`doc.GetElement(el.GetTypeId())`). Все геттеры параметров камеры
  (дальность, высота, наклон, поворот, фокусное расстояние, формат
  матрицы) идут через эту функцию — на практике одни из этих
  характеристик естественно держать параметром типа (дальность — общая
  характеристика модели камеры), другие — экземпляра (высота установки,
  наклон — у каждой камеры свои; фокусное расстояние — почти всегда
  экземпляра у вариофокального объектива, подстроенного по месту), а
  часть проектов держит их иначе — функция не требует знать заранее, где
  что лежит.
- `_optical_fov(doc, fi, settings)` — горизонтальный И вертикальный угол
  ...
  статус `no_optics` у `_one_camera`.
- `build_fov_zones(doc, cameras, view, settings)` — главный вход, вызывается
  скриптом кнопки внутри транзакции. По каждой камере: угол обзора из
  `_optical_fov` (см. выше); дальность — из параметра камеры; направление
  ...
  построенных зон `detail` — сводка (азимут, угол, R, конус (`cone_note`
  — «h=… наклон=… верт.угол=…» либо какого из двух не хватает —
  вертикальный угол теперь всегда есть, если есть hfov), источник
...
- Учёт высоты установки: `_mounting_height_ft` (из параметра или из отметки
  ...
  плоскость расчёта (вертикальный угол уже посчитан `_optical_fov`):
  `near = h / tg(наклон + вертикальный/2)`,
  `far = h / tg(наклон − вертикальный/2)` (при `наклон ≤ вертикальный/2`
  дальняя граница = дальность). Без высоты и/или наклона — плоский сектор
  `(0, дальность)`, как в исходном Dynamo-скрипте.
- Обрезка по помещению из связи: `room_rings_for_point(doc, точка_с_Z,
  режим)` перебирает загруженные `RevitLinkInstance`, для каждого `Room`
  тесселлирует контур (`Finish`, откат на `Center`), переводит в
  координаты хоста и делает **2D-тест «точка внутри по XY»** (не
  `IsPointInRoom` — тот отсекал камеру под потолком, т.к. чувствителен к
  Z). При нескольких попаданиях по XY (этажи стопкой) выбирает по
  вертикальному габариту `Room` (`get_BoundingBox` в координатах связи,
  Z камеры туда же); вне контура — ближайший в пределах 90 мм. Возвращает
  `(контуры|None, пояснение)` — пояснение уходит в отчёт (нет связей / не
  загружены / нет помещений / точка вне всех контуров / из скольких по
  XY выбрано). Если `GetBoundarySegments` не отдал ни одного пригодного
  контура (разомкнутая/повреждённая граница) — `_room_boundary_rings`
  откатывается на `_room_solid_rings`: реальная геометрия, работает для
  помещений любой формы.
- Общая 3D-геометрия (нужна только как резерв для помещений):
  `_element_solids(el, opts)` — солиды элемента, включая геометрию внутри
  `GeometryInstance` (семейства); `_horizontal_face_rings(solid, tf)` —
  контур нижней горизонтальной плоской грани тела в координатах хоста
  (для призмы это и есть контур в плане). Тени от колонн отдельно НЕ
  считаются: колонна с отделкой, отмеченной «граница помещения» (обычная
  практика в АР), уже вырезает свой контур внутри границы `Room`, и
  `GetBoundarySegments` отдаёт его дополнительным внутренним контуром —
  этого достаточно, отдельная обработка категорий колонн из модели
  убрана как избыточная (и как источник собственных багов — камера,
  смонтированная на колонне, или Revit, «сшивающий» geometry колонны с
  соседним перекрытием, портили контур).
- `_zone_curveloop` строит звёздный многоугольник от точки камеры (или
  кольцевой сектор при мёртвой зоне), обрезая **каждый луч** по
  `_clip_distance` по ВСЕМ контурам помещения (внешнему и внутренним —
  «дырам» от колонн — одинаково) — работает с вогнутыми помещениями и
  несколькими «дырами» сразу. Соседние точки контура склеиваются порогом
  `_MIN_SEG_FT` (~3 мм, с запасом выше `Application.ShortCurveTolerance`
  Revit) — раньше здесь стоял порог на два порядка меньше минимальной
  длины кривой, которую примет `Line.CreateBound`, и при частой сетке
  вершин (сложная граница помещения с несколькими внутренними контурами)
  один такой отрезок обрывал построение всего контура; теперь проблемный
  отрезок пропускается, не отменяя остальное. Если обрезка всё равно
  выродила контур — `_one_camera` строит зону без неё (статус
  `ok_clip_failed`) вместо отказа.
- Идемпотентность: одна `FilledRegion` на камеру, в «Комментарии» пишется
  `<zone_tag>|<Id камеры>`; `_delete_previous` удаляет прежние зоны
  выбранных камер на этом виде (и дуги DORI — по имени стиля линии
  `«<line_subcategory> · DORI»`) перед перестроением.
- Зоны DORI (EN 62676-4) — `_draw_dori`, необязательно: дуги на расстояниях
  идентификации/распознавания/наблюдения/обнаружения (250/125/63/25 пикс/м),
...
- `auto_aim_cameras(doc, cameras, view, settings)` / `_auto_aim_one` —
  ...
  пишет в `tilt_param_name`. Высота — параметр камеры
  (`_mounting_height_ft`) или, если не нашлась, `auto_default_height_mm`
  — это значение тоже ЗАПИСЫВАЕТСЯ в `height_param_name` (если он есть на
  экземпляре), чтобы «Зоны обзора» потом взяли ту же высоту. Вертикальный
  угол — из `_optical_fov` (расчётный, как всегда) или, если оптика не
  настроена/не нашлась на этой камере, `auto_default_vfov_deg` — этот
  запасной вариант **не записывается никуда** (это расчётный вход, не
  параметр камеры), используется только для расчёта наклона в этот раз.
  `tilt_param_name` ищется через голый `cam.LookupParameter` (НЕ
  `_find_param`) — намеренно: писать наклон нужно per-instance, и если он
  существует только параметром типа, кнопка отказывается
  (`tilt_not_instance`), чтобы не переставить наклон разом у всех камер
  этого типа в разных помещениях.
...
семейства до следующего этажа того же корпуса (по имени уровня), по ширине — габарит геометрии ± 2000 мм,
...
| `base_level(doc, el, fallback_z)` | Базовый уровень (LevelId → ReferenceLevel → уровневые параметры → основа → по отметке) |
