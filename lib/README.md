...
| `trunk_lane_x` | `trunk_lane_x(component_index, panel_count)` | X отдельной вертикальной "дорожки" магистральной ЦЕПОЧКИ с этим номером — с заметным зазором (`TRUNK_LANE_GAP_MM`, больше `RISER_SPACING_MM`) после стояка последней панели, а не вплотную к нему (иначе переход к дорожке выглядел как маленький отросток у стояка панели, а не как отдельная линия); между собой дорожки разных цепочек — с обычным шагом `RISER_SPACING_MM`. Чистая функция без Revit API |
...
физически за пределами уже нарисованной рамки этажа), `ROOM_BOTTOM_MM`
