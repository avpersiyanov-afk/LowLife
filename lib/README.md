...
`BOUNDARY_MARGIN_MM`(8мм) — первый стояк на `BOUNDARY_MARGIN_MM` ПРАВЕЕ
самой левой линии рамки этажа, а не ровно на ней — раньше совпадал с
ней в точности, что и выглядело как "стояк сидит прямо на границе
рамки"), `RISER_SPACING_MM` (8мм между стояками разных панелей —
масштаб схемы, не 300мм, как было в первой версии: с шагом 300мм при
масштабе схемы "20мм между устройствами" коллектор до дальнего стояка
тянулся через явно пустой промежуток, а стояк оказывался физически за
пределами уже нарисованной рамки этажа), `LEVEL_FRAME_WIDTH_MM` (35мм —
то же число, что `LEVEL_LINE_1_OFFSET_MM + LEVEL_LINE_2_OFFSET_MM +
LEVEL_LINE_3_OFFSET_MM` у СОТ: ширина "коридора" трёх левых линий рамки,
которая уже есть без дополнительного расширения — см. `extra_left_mm`
ниже), `ROOM_BOTTOM_MM`
...
Тот же принцип, что и "Место под шину" выше, только по X, а не по Y: у
СОТ третья (самая левая) линия рамки этажа стоит на фиксированном месте
(`LEVEL_LINE_1_OFFSET_MM + LEVEL_LINE_2_OFFSET_MM + LEVEL_LINE_3_OFFSET_MM
= 35мм` левее рамок помещений, см. `LEVEL_FRAME_WIDTH_MM` в
`scs_schematic.py`) — этого не хватает, если у СКС стояков панелей и
дорожек магистралей больше, чем помещается в эти 35мм с запасом
`BOUNDARY_MARGIN_MM` с каждой стороны. `sync_levels`/`_draw_level_frame`
получили новый **опциональный** параметр `extra_left_mm=0.0`: отодвигает
третью линию рамки этажа ещё левее на это число мм (первая и вторая
линии, ближе к помещениям, не трогаются — привязаны к рамкам помещений
и подписи этажа между ними, которая всё ещё центрируется между второй и
[уже отодвинутой] третьей линией). При `extra_left_mm=0.0` (СОТ/СПС не
передают этот аргумент вовсе) — поведение побайтово то же, что раньше,
обратная совместимость сохранена. Как и `extra_bottom_mm` — общий на все
этажи, сравнивается с сохранённым в `state["extra_left_mm"]`, форсирует
...
уже доступные без расширения `LEVEL_FRAME_WIDTH_MM`.
