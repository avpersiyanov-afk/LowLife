...
`LAYOUT_GROUPED` (`_arrange_grouped`) — по типам (`_group_by_type`):
маленькие типы (кабелей **≤ 8**, `BUNDLE_SIZE`) — пирамидками на полу лотка
слева направо (`_pyramid_offsets`: 2 — рядом, 3 — пирамидка, 4 — 3 снизу +
1 сверху; неполный верхний ряд центрируется сдвигом кратным 2r). Большие
типы (**> 8**) — стянуты в пучки по ~8 «ромашкой» (`_split_bundles` — без
крошечного хвоста: 10→[5,5], 11→[6,5]; `_daisy_offsets`: 2 — рядом, 3 —
треугольник, 4 — квадрат, 5–8 — центр + кольцо), и каждая ромашка ПАДАЕТ
bottom-left в свободное место лотка поверх уже уложенного (`_blf_pack` с
`obstacles`, `keep_order=True`) — заполняет пустоты над короткими кучками и
по бокам, не обрезается по высоте, ничего не висит в воздухе. Маленький тип,
не влезший на пол, тоже добирается `_blf_pack`. `ties` из `plan_section` —
кольца-стяжек, `draw_section` их рисует. BLF-движок общий —
`_blf_pack(radii, w, h, obstacles=None, keep_order=False)`.
