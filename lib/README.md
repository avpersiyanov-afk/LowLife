...
ограничивать, как раньше; и `sync_rooms_in_level`, и `sync_levels`, тем
же значением по умолчанию — СОТ/СПС его не передают, для них ничего не
меняется). Без ограничения все помещения этажа шли в ОДНУ строку слева
...
СКС — единственная дисциплина, которая пока передаёт `max_row_width_mm`
(поле `[Схема] Максимальная ширина строки помещений на этаже, мм` в
«Параметры СКС», `BuildScsSchematic.pushbutton/script.py`).
