...
Базовые и вторые линии выбираются кликом одна за другой БЕЗ отдельного
подтверждения «Готово» (`pick_lines_loop` — цикл одиночных `PickObject`,
Esc заканчивает шаг) вместо `PickObjects` с Enter/«Готово» в конце.
Сторона обхода ОДНА на весь запуск (`ask_side_reference` — один `PickPoint`
после выбора всех линий), не своя у каждой линии/пролёта: та же точка
используется, чтобы определить сторону у КАЖДОГО пролёта на КАЖДОЙ второй
линии (`build_curves_for_side` — знак скалярного произведения
перпендикуляра пролёта на вектор к этой точке) — для группы примерно
параллельных линий это и даёт "все обходят в одну сторону".
...
Результат сначала строится как ПРЕВЬЮ прямо в модели, внутри одной
открытой (ещё не закоммиченной) `Transaction` с ручными `Start`/`Commit`/
`RollBack` — тот же приём, что уже используется в проекте (см.
`circuit_highlight.py`/`family_catalog.py`): пользователь видит результат
и либо принимает его (`Commit`), либо кликает другую точку стороны —
тогда предпросмотр удаляется и перестраивается заново той же ещё открытой
транзакцией (без повторного удаления оригиналов вторых линий). Явная
отмена на этом шаге откатывает транзакцию целиком (`RollBack`) —
оригинальные вторые линии, удалённые при первом построении превью,
восстанавливаются как будто ничего не запускалось. Держать транзакцию
открытой поперёк нескольких `PickPoint`/`forms.alert` — приём из реальных
Revit-плагинов с живым предпросмотром, но именно эта связка в этом
проекте раньше не проверялась на практике (нет живого Revit, см.
CLAUDE.md) — если предпросмотр поведёт себя не так, как ожидается, это
первое место, на которое стоит посмотреть.
...
| `highlight_elements` | `highlight_elements(uidoc, elements)` | Выделяет элементы в модели (`SetElementIds`) — перед `ask_side_reference`, чтобы было видно, какие линии затронуты |
| `pick_lines_loop` | `pick_lines_loop(uidoc, doc, selection_filter, prompt, exclude_ids=None)` | Цикл одиночных `PickObject` без подтверждения «Готово» между кликами — Esc заканчивает; пропускает дубли и `exclude_ids` |
| `ask_side_reference` | `ask_side_reference(uidoc, prompt)` | Один `PickPoint` — точка, определяющая сторону обхода для ВСЕХ линий и пролётов; `None`, если отменено |
...
| `compute_line_plan` | `compute_line_plan(base_lines, second_el, radius_ft, bundle_mode)` | Считает пролёты второй линии (без выбора стороны); `(line_plan, error)` — `line_plan` — геометрия, достаточная для `build_curves_for_side` под ЛЮБУЮ сторону, либо `None` с причиной пропуска (нет пересечений, диаметр/запас не помещается, пролёты пересекаются в «Одиночном» режиме) |
| `build_curves_for_side` | `build_curves_for_side(line_plan, side_pt)` | Строит список `Line`/`Arc` (прямая-дуга-...-прямая) для `line_plan` под общую точку стороны `side_pt` |
| `run_line_bypass_tool` | `run_line_bypass_tool(doc, uidoc, view)` | Весь сценарий кнопки: диаметр → режим → базовая(-ые) линия(-и) → вторая(-ые) линия(-и) → общая сторона → превью в открытой транзакции с перевыбором стороны → `Commit` |
