...
выбирается «базовая» линия (остаётся целой), затем одна ИЛИ несколько
«вторых» линий за один `PickObjects` — каждая из них в точке пересечения с
базовой разрывается и соединяется дугой уже известного диаметра, без
повторного запроса. Сторона обхода не выводится жёстким правилом —
пользователь указывает её сам кликом (`PickPoint`) отдельно для каждой
второй линии. Диаметр — буквально диаметр дуги: на второй линии вырезается
...
равноудалены от точки пересечения на `radius_ft`, поэтому точка
пересечения — центр окружности, а `Arc.Create` всегда даёт полуокружность).
...
| Функция | Сигнатура | Что делает |
|---|---|---|
| `ask_diameter_mm` | `ask_diameter_mm()` | Диалог ввода диаметра дуги, мм; `None`, если отменено |
...
| `plan_bypass` | `plan_bypass(uidoc, base_p1, base_p2, second_el, radius_ft)` | Считает точки разрыва второй линии и спрашивает (`PickPoint`) сторону обхода; `(plan, error)` — `plan` для `apply_bypass_plan`, либо `None` с причиной пропуска (нет пересечения, диаметр не помещается, отмена выбора стороны) |
...
| `run_line_bypass_tool` | `run_line_bypass_tool(doc, uidoc, view)` | Весь сценарий кнопки: диаметр → базовая линия → вторая(-ые) линия(-и) → обходы |
